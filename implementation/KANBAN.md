...
credentials, and an independent security reviewer. Do not remove
...
Native APNs/FCM, self-hosted TURN support, encrypted WebRTC signaling, and the
Android debug build are implemented. Completion requires:
...
Local verification on 2026-07-29 passed `scripts/test.ps1` and
`scripts/lint.ps1` after fixing the DM block-action widget test to scroll its
lazy list before interacting with the action.
...
| Android reproducible release build | Pending | Requires signing approval |
